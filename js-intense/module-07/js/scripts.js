...
const methods = {
  ...
  getUsersByGender(users, gender) {
    const filteredByGender = users.filter(user => user.gender === gender)
    return filteredByGender
  },

  getInactiveUsers(users) {
    const filteredByActivity = users.filter(user => user.isActive === false);
    return filteredByActivity;
  },

  getUsersByEmail(users, email) {
    const filteredByEmail = users.filter(user => user.email === email);
    return filteredByEmail;
  },

  getUsersWithAge(users, min, max) {
    const filteredByAge = users.filter(user => user.age >= min && user.age <= max);
  ...
  getUsersByFriend(users, friend) {
    const filteredByFriend = users
